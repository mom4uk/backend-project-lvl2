...
const genDiff = (filepath1, filepath2, formatName) => {
  ...
  const iter = (content1, content2) => {
    const sharedKeys = uniq([...keys(content1), ...keys(content2)]).sort();//use flatMap ?
    return sharedKeys.reduce((acc, key) => {
      const value1 = content1[key];
      const value2 = content2[key];
      if (has(content2, key) && isObject(value1) && isObject(value2)) {
      ...
      && value1 !== value2 && (!isObject(value1) || !isObject(value2))) {
        acc.push({
